...
describe("ProductController", () => {
    ...
    beforeEach(async () => {
        ...
        productService = module.get(product_service_1.ProductService);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe("calculatePrices", () => {
        it("deve calcular preços corretamente", async () => {
            ...
            mockProductService.calculatePrices.mockResolvedValue(mockCalculatedPrices);
            const calculatePricesDto = {
                cost: 4500,
                freight: 100,
            };
            const result = await controller.calculatePrices(calculatePricesDto);
            expect(productService.calculatePrices).toHaveBeenCalledWith(calculatePricesDto);
    ...
    describe("findAll", () => {
        ...
        it("deve retornar produtos agrupados por categoria", async () => {
            ...
            const result = await controller.findAll(undefined, undefined, true, "category");
            expect(productService.findAll).toHaveBeenCalledWith({
                category: undefined,
                isNew: undefined,
                isActive: true,
    ...
    describe("findByCategory", () => {
        it("deve retornar produtos por slug da categoria", async () => {
            ...
            expect(productService.findByCategory).toHaveBeenCalledWith("iphones-novos");
            expect(result).toEqual(mockProducts);
        });
    });
    describe("create", () => {
        it("deve criar produto com usuário autenticado", async () => {
            ...
            expect(productService.create).toHaveBeenCalledWith(createProductDto, "user-123");
            expect(result).toEqual(mockProduct);
        });
    });
});
