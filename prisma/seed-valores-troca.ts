...
async function seedValoresTraca() {
  ...
  // Valores de exemplo (você deve ajustar conforme sua tabela real)
  const valoresTraca = [
    // iPhone 11 Series
  ...
  for (const valor of valoresTraca) {
    try {
      ...
      console.log(
        `✅ ${valor.modelo} ${valor.capacidade} - R$ ${valor.valorBase.toFixed(2)}`
      );
    } catch (error) {
      console.error(
        `❌ Erro ao inserir ${valor.modelo} ${valor.capacidade}:`,
        error
      );
    }
  }
